...
2) Download facility information from API with append-only behavior
3) Fetch agency list from API
4) Iterate agency content details and download immediately when a new file is found
5) Stop after --limit newly downloaded files (limit counts new downloads only)
6) Parse newly downloaded PDFs to parquet text (pdf_parsing)
7) Write cumulative metadata CSV and run-specific metadata output CSV
"""
...
def load_existing_facility_data(csv_path: str) -> Dict[str, Dict[str, str]]:
    if not os.path.exists(csv_path):
        return {}
    ...
    with open(csv_path, mode="r", encoding="utf-8", newline="") as fh:
        reader = csv.DictReader(fh)
        for row in reader:
            license_number = (row.get("LicenseNumber") or "").strip()
            if not license_number:
                continue
            existing_data[license_number] = {
                column: row.get(column, "")
                for column in FACILITY_INFO_COLUMNS
            }
    return existing_data


def update_facility_information_csv(all_agency_info: Dict[str, object], csv_path: str) -> int:
    """
    Append-only facility information sync.
    - Keeps all facilities already present in CSV.
    - Updates/adds facilities present in API response.
    - Marks LicenseStatus='Unknown' for facilities that remain in CSV but are absent in API.
    """
    existing_data = load_existing_facility_data(csv_path)
    ...
    for agency in agency_list:
        ...
        if license_number not in existing_data:
            appended_count += 1

        existing_data[license_number] = {
            column: agency.get(column, "")
            for column in FACILITY_INFO_COLUMNS
        }

    unknown_count = 0
    for license_number, row in existing_data.items():
    ...
    with open(csv_path, mode="w", newline="", encoding="utf-8") as fh:
        writer = csv.DictWriter(fh, fieldnames=FACILITY_INFO_COLUMNS, quoting=csv.QUOTE_ALL)
        writer.writeheader()
    ...
    print(
        ...
        f"(total={len(existing_data)}, appended={appended_count}, marked_unknown={unknown_count}): {csv_path}"
    )
    return len(agency_list)


def parse_new_downloads_to_parquet(new_rows: List[Dict[str, str]], parquet_dir: str, download_dir: str) -> None:
...
def main() -> None:
    ...
    facility_info_csv = os.path.join(root_dir, DEFAULT_FACILITY_INFO_CSV)
    metadata_csv = (
    ...
    # Step 3: Update facility information CSV with append-only logic
    agency_count_for_facility_step = update_facility_information_csv(all_agency_info, facility_info_csv)
    print(f"Fetched {agency_count_for_facility_step} agencies for facility information update.")
    ...
    if not args.skip_pdf_parsing:
        parse_new_downloads_to_parquet(new_rows, parquet_dir, download_dir)
